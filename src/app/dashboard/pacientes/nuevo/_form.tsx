  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <form action={action} className="space-y-8">
        ...
        {isSocialIntegration && (
          <section className="bg-white rounded-2xl border border-warm-200 p-6 space-y-5">
            <h2 className="text-lg font-semibold text-warm-900">Integración social</h2>
            <div>
              ...
              >
                <option value="">Seleccionar...</option>
                {disabilityTypeOptions.map((o) => (
                  <option key={o.value} value={o.value}>{o.label}</option>
                ))}
              </select>
            </div>
            <Field label="Nivel de autonomía" name="autonomyLevel" />
            <TextArea label="Contexto de integración" name="integrationContext" />
          </section>
        )}
