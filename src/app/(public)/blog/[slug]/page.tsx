...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <article className="min-h-screen bg-white">
      {/* Header */}
      <header className="bg-gradient-to-br from-sage-50 to-warm-50 py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl mx-auto">
          <Link
            href="/blog"
            className="inline-flex items-center gap-1.5 text-warm-500 hover:text-sage-600 text-sm mb-8 transition-colors"
          >
          ...
          <div className="flex flex-wrap items-center gap-3 mb-5">
            ...
            {post.tags?.map((tag) => (
              <span key={tag} className="text-xs bg-sage-100 text-sage-700 px-2.5 py-0.5 rounded-full">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
        <div className="prose prose-warm max-w-none
          ...
          prose-li:my-1
          prose-blockquote:border-l-sage-400 prose-blockquote:text-warm-600 prose-blockquote:bg-sage-50 prose-blockquote:py-1 prose-blockquote:px-4 prose-blockquote:rounded-r-lg
          prose-hr:border-warm-200
          prose-code:text-sage-700 prose-code:bg-sage-50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded">
          <MDXRemote source={post.content} />
        </div>

        {/* Author card */}
        <div className="mt-16 pt-10 border-t border-warm-100">
          <div className="bg-sage-50 rounded-2xl p-6 flex gap-5 items-start">
            <div className="w-14 h-14 bg-sage-200 rounded-full flex items-center justify-center text-2xl flex-shrink-0">
              👩‍⚕️
            </div>
            <div>
