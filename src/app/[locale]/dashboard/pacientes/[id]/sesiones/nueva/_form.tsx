...
export default function NuevaSesionForm({
  ...
  const initialMeetLink = patientDefaults?.meetLink ?? "";
  const initialMeetProvider = patientDefaults?.meetProvider ?? "";
  const initialAmount = patientDefaults?.defaultAmount ?? "";

  const [notes, setNotes] = useState("");
  const [isOnline, setIsOnline] = useState(Boolean(initialMeetLink));
  const [meetLink, setMeetLink] = useState(initialMeetLink);
  const [meetProvider, setMeetProvider] = useState(initialMeetProvider);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                step="0.01"
                min={0}
                defaultValue={initialAmount === "" ? undefined : initialAmount}
        ...
        <div className="bg-white rounded-2xl border border-warm-200 p-6">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              className="w-4 h-4 rounded border-warm-300 text-sage-600 focus:ring-2 focus:ring-sage-500"
            />
            <span className="text-sm font-medium text-warm-900">{t("isOnline")}</span>
          </label>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-warm-200 p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select id="meetProvider" name="meetProvider" value={meetProvider} onChange={(e) => setMeetProvider(e.target.value)} disabled={!meetLink} className="w-full px-4 py-2.5 rounded-lg border border-warm-300 focus:ring-2 focus:ring-sage-500 focus:border-sage-500 outline-none text-sm disabled:opacity-50 disabled:bg-warm-50">
