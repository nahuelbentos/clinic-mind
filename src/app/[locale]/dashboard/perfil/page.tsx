...
export default async function PerfilPage() {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { name: true, email: true, profession: true, licenseNumber: true, plan: true, createdAt: true, locale: true },
  });
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <span className="text-xs px-2 py-0.5 rounded-full bg-sage-100 text-sage-700 font-medium">
                {t("plan", { plan: user.plan })}
              </span>
              <span className="text-xs text-warm-400">
                {t("since", { date: new Date(user.createdAt).toLocaleDateString("es-AR") })}
              </span>
            </div>
          </div>
        </div>
