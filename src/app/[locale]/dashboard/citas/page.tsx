...
export default async function CitasPage() {
  ...
  const statusLabels: Record<string, { label: string; style: string }> = {
    CONFIRMED: { label: tAppt("statusConfirmed"), style: "bg-sage-100 text-sage-700" },
    PENDING: { label: tAppt("statusPending"), style: "bg-cream-100 text-cream-700" },
    CANCELLED: { label: tAppt("statusCancelled"), style: "bg-red-100 text-red-700" },
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {appointments.length === 0 ? (
        <div className="bg-white rounded-2xl border border-warm-200 p-12 text-center">
          <p className="text-warm-500">{tAppt("noAppointments")}</p>
        </div>
      ) : (
